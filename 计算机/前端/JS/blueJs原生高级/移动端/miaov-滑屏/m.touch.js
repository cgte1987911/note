...
function swiper(init){
	...
	wrap.addEventListener('touchmove', function(e) {
		...
		if(isFirst){
			 ...
			 if(init.slideObj=='screen'){
				var flag=false;
				for(var i=0;i<init.conflictObj.length;i++){
					var confObjs=init.conflictObj[i].getElementsByTagName("*");
					confObjs[confObjs.length]=init.conflictObj[0];
					confObjs.length+=1
					for(var j=0;j<confObjs.length;j++){
						if(e.target==confObjs[j]){
							flag=true;
							break;
						}
			...
			isFirst = false;
			function SetDir(offset){
				var offsetValue=offset?parseInt(offset):0
				if(Math.abs(dis.x) > Math.abs(dis.y)+offsetValue) {
	...
	wrap.addEventListener('touchend', function(e) {
		...
		init.end&&init.end.call(wrap,e,startEl);
	});
	function getF(over){ 
